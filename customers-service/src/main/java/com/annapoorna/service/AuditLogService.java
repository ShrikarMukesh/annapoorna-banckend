...
import com.annapoorna.entity.AuditLog;
import com.annapoorna.repository.AuditLogRepository;
...
public class AuditLogService {
    ...
    @Value("${spring.application.name:customers-service}")
    ...
    @Async
    public void logRequest(String eventType, String entityType, String entityId,
            String userId, Object requestPayload, Object responsePayload,
            Integer statusCode, String description, String ipAddress) {
        try {
            AuditLog auditLog = AuditLog.builder()
                    .serviceName(serviceName)
                    .eventType(eventType)
            ...
            auditLogRepository.save(auditLog);
            log.debug("Audit request log saved: {} - {} - {}", eventType, entityType, entityId);
        } catch (Exception e) {
            log.error("Failed to save audit request log: {}", e.getMessage(), e);
        }
    }
